...
In the beginning, we filtered out the data that was irrelevant to this research. The dropped columns can be seen in @lst-del:
...
The scanpath visualization (see @fig-scanpath) was used to trace the path that a subject's eye movements took to ensure alignment with the expected gaze patterns. The heatmaps (see @fig-heatmap) allowed quick verification of attention directed towards the defined areas of interest (AOI).
...
With these two introduced metrics, a variety of interesting analyses and visualisations is possible. The target and non-target ratio can be looked at for an individual subject and sentence with a specific condition (see @fig-trntrstatic). 
